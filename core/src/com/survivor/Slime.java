...
public class Slime {
    ...
    private static final float DEFAULT_SPAWN_INTERVAL = 4;
    ...
    public void update(SpriteBatch batch, float delta, Player player) {
        ...
        // check if Slime has been struck, or player has died
        ...
        if (!attacking && lastAttackTimer > ATTACK_COOLDOWN) {
            // attack to the left
            if (centreX - 1.5f * WIDTH < playerX && playerX < centreX) {
                rightFacing = false;
            ...
            // attack to the right
            if (centreX < playerX && playerX < centreX + 1.5f * WIDTH) {
                rightFacing = true;
                velocity.x = ATTACK_SPEED;
                attacking = true;
                animTimer = 0;
    ...
    public float getCentreX() {
        return renderPosition.x + renderPosition.width / 2;
    }
    ...
    public static void create() {
        ...
        spriteSheet.loadAnim("die", 0.3f, 16, 17, 18, 19, 19, 20, 20);
    ...
    public static Slime collidedSlime(Rectangle rectangle) {
        for (Slime slime : slimes) {
            if (!slime.dying && !slime.spawning && slime.attacking &&
